...
// var ProcessDiagramCanvas = {
	...
	*/
	...
	onClick: function(event, instance, element){
		...
		var contextObject = ProcessDiagramGenerator.getContextObject(instance.processDefinitionId, element.id);
	...
	onHoverIn: function(event, instance, element){
		...
		var border = instance.g.getById(element.id + "_border");
		if (border)
	...
	addHandlers: function(set, x, y, width, height, type){
		...
		if (type == "event") {
			...
			var overlay = this.g.rect(x, y, width, height);
		}
		...
		overlay.attr({stroke: Color.Orange,"stroke-width": 3, fill: Color.get(0,0,0), opacity: 0.0, cursor: "pointer"});
	...
	_drawFlow: function(waypoints, conditional, isDefault, highLighted, flowDirection, connectionType){
		var originalPaint = this.getPaint();
...
// TODO: generate polylineId or do something!!
		...
		polyline.element.polyline = polyline;
		...
		var tailLine = polyline.getTailLine(); //back direction
		...
		if(flowDirection == "One" || flowDirection == "Both")
			arrowHead = this._drawArrowHead(line, connectionType);
		if (flowDirection == "Both")
			arrowTail = this._drawArrowHead(tailLine, connectionType);
		...
		if (conditional) {
			conditionalSequenceFlowIndicator = this._drawConditionalSequenceFlowIndicator(tailLine);
		}
		
		// Add mouse-event handlers
		
		var st = this.g.set();
		st.push(polyline.element, arrowHead, arrowTail, circleTail, conditionalSequenceFlowIndicator, defaultSequenceFlowIndicator);
		polyline.element.data("set", st);
		polyline.element.data("flowDirection", flowDirection);
		polyline.element.data("connectionType", connectionType);
		
		// NOTE: for path in IE opacity have to be more than 0 to have handler "on mouse over"
		var polyCloneAttrNormal = {"stroke-width": this.getStroke() + 5, stroke: Color.get(132,112,255), opacity: 0.01, cursor: "pointer"};
		...
		polyClone.click(function(){
			...
			if (contextObject) {
				console.log("[flow], objectId: %o, flow: %o, contextObject: %o", object.id, contextObject.toString(), contextObject);
		...
		// Stylization by connection type
	...
	_drawDefaultSequenceFlowIndicator: function(line) {
		...
		this.updateDefaultIndicatorPosition(defaultIndicator, line);
		
		return defaultIndicator;
	},
	
	drawSequenceflow: function(waypoints, conditional, isDefault, highLighted) {
		var flowDirection = "One";
		this._drawFlow(waypoints, conditional, isDefault, highLighted, flowDirection, CONNECTION_TYPE.SEQUENCE_FLOW);
	},
	
	drawMessageflow: function(waypoints, highLighted) {
		var flowDirection = "One";
		var conditional=isDefault=false;
		this._drawFlow(waypoints, conditional, isDefault, highLighted, flowDirection, CONNECTION_TYPE.MESSAGE_FLOW);
	},
	
	drawAssociation: function(waypoints, flowDirection, highLighted) {
		var conditional=isDefault=false;
		this._drawFlow(waypoints, conditional, isDefault, highLighted, flowDirection, CONNECTION_TYPE.ASSOCIATION);
	},
	...
	_drawArrowHead: function(line, connectionType){
		...
		this.updateArrowPosition(arrowHead, line);
	...
	updateArrowPosition: function(arrowHead, line) {
		// anti smoothing
		if (this.strokeWidth%2 == 1)
			line.x2 += .5, line.y2 += .5;
	...
	updateConditionalIndicatorPosition: function(conditionalIndicator, line) {
		// hook for IE
		var d = ($.browser.msie)? -1.5 : 0;

		conditionalIndicator.transform("t" + line.x2 + "," + line.y2 + "");
		conditionalIndicator.transform("...r" + Raphael.deg(line.angle - Math.PI / 2) + " " + d + " " + d);
	},
	updateCircleTailPosition: function(circleTail, line) {
		circleTail.attr({cx: line.x1, cy: line.y1});
	},
	updateDefaultIndicatorPosition: function(defaultIndicator, line) {
		var len = 10; c = len/2, f = 8;
	...
	*/
	...
	_drawConditionalSequenceFlowIndicator: function(line){
		...
		*/
		...
		this.updateConditionalIndicatorPosition(polygone.element, line);
	...
	_drawAnnotationText: function(text, x, y, width, height){
	...
	drawLabel: function(text, x, y, width, height){
		...
		var set = this.g.setFinish();
		var rect = this.g.rect(x, y, width, height);
		rect.id = contextObject.id + "_label";
		rect.set = set;
	},
	...
	*/
	...
	highLightFlow: function(flowId){
		...
		if (contextObject)
			console.log("--> highLightFlow: [%o] %o", contextObject.id, contextObject.toString());
		...
		// fill arrowHead for sequence flow
		var connectionType = shapeFlow.data("connectionType");
		if (connectionType == CONNECTION_TYPE.SEQUENCE_FLOW)
	...
	drawTextAnnotation: function(text, xx, yy, width, height){
		this.g.setStart();
      ...
      // for debug
	  if (!this.debug) {
          var rectStyle = {stroke: Color.get(112, 146, 190), "stroke-width": 1.0, "stroke-dasharray": "- "};
          var r = this.g.rect(boxX-2, boxY-2, boxWidth+4, boxHeight+4);
		  r.attr({fill: Color.white, "fill-opacity": 0.3, stroke: Color.get(112, 146, 190), "stroke-width": 1.0, "stroke-opacity": 0.2});
		  //r.attr(rectStyle);
	  }
	  //
      
	  this._drawAnnotationText(text, boxX, boxY, boxWidth, boxHeight);
	  
	  var set = this.g.setFinish();
	  this.addHandlers(set, xx, yy, width, height, "annotation");
	},
