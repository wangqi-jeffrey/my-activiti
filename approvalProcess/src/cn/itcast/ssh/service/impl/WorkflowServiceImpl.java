...
//		String key = leaveBill.getClass().getSimpleName();
		...
		variables.put("inputUser", String.valueOf(SessionContext.get().getId()));//表示惟一用户
		/**
	...
	@Override
	public List<Task> findTaskListByName(Long name) {
		List<Task> list = taskService.createTaskQuery()//
					.taskAssignee(String.valueOf(name))//指定个人任务查询
	...
	public List<String> findOutComeListByTaskId(String taskId) {
		...
		List<HistoricTaskInstance> list = historyService.createHistoricTaskInstanceQuery()//创建历史任务实例查询
				...
				.list();
		System.out.println("---------------------获取历史任务start-----------------");
	...
	private List<String> nextTaskLineNames(ActivityImpl activityImpl) {
        ...
        // 当前任务之后没有连线
        if (outTransitions == null || outTransitions.size() == 0) return null;
        
        // 只有一条连线并且名称为空
        if (outTransitions.size() == 1) {
        	PvmTransition pvm = outTransitions.get(0);
        	PvmActivity pvmAc = pvm.getDestination();
        	if ("userTask".equals(pvmAc.getProperty("type")) && (pvm.getProperty("name") == null || "".equals(((String) pvm.getProperty("name")).trim()))) {
        		list.add("默认提交");
        		return list;
			}
		}
        ...
        for (PvmTransition tr : outTransitions){
            ...
            if ("exclusiveGateway".equals(ac.getProperty("type"))) {  
                ...
                if (outTransitionsTemp == null || outTransitionsTemp.size() == 0) continue;
                // 只获取一级，一步一步往后执行
...
//              taskDefinition.getCandidateGroupIdExpressions().toArray();  
                ...
                for(PvmTransition tr:outTransitions){    
                    ...
                    if("exclusiveGateway".equals(ac.getProperty("type"))){  
                        ...
                        System.err.println(ac.getProperty("type"));  
                    }  
                }   
            return null;  
        }  
...
//		String objectName = leaveBill.getClass().getSimpleName();
		String objectName = "ApprovalProcess";
		//组织流程表中的字段中的值
